...
struct ResidualBlockImpl : public torch::nn::Module {
	...
	ResidualBlockImpl(int inputs, int filters, int outputs) {
		register_module("conv1",
						conv1 = torch::nn::Conv2d(torch::nn::Conv2dOptions(inputs, filters, 3).padding(1).stride(1)));
		register_module("batchNorm1", batchNorm1 = torch::nn::BatchNorm2d(filters));

		register_module("conv2",
						conv2 = torch::nn::Conv2d(torch::nn::Conv2dOptions(filters, outputs, 3).padding(1).stride(1)));
