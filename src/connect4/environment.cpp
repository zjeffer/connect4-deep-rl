...
Environment::Environment(torch::Tensor board, ePlayer currentPlayer)
	: m_Board(board.detach().clone()), m_Rows(board.size(0)),
	  m_Cols(board.size(1)), m_CurrentPlayer(currentPlayer) {}

Environment::Environment(const std::shared_ptr<Environment> &other)
	: m_Board(other->m_Board.detach().clone()), m_Rows(other->m_Rows),
	  m_Cols(other->m_Cols), m_CurrentPlayer(other->m_CurrentPlayer),
...
ePlayer Environment::getCurrentPlayer() const { return m_CurrentPlayer; }

void Environment::setCurrentPlayer(ePlayer player) { m_CurrentPlayer = player; }

void Environment::togglePlayer() {
	setCurrentPlayer(m_CurrentPlayer == ePlayer::YELLOW ? ePlayer::RED
														: ePlayer::YELLOW);
}
...
bool Environment::isValidMove(int column) const {
	...
	throw std::runtime_error(
		"Error: isValidMove(): value is not 0, 1 or 2, but: " +
		std::to_string(value));
}

int Environment::getRows() const { return m_Rows; }

int Environment::getCols() const { return m_Cols; }

ePlayer Environment::getPlayerAtPiece(int row, int column) const {
	return static_cast<ePlayer>(m_Board[row][column].item<int>());
}

torch::Tensor Environment::getBoard() { return m_Board; }
...
bool Environment::currentPlayerHasConnected4() const {
	...
	while (col - amountLeftOfCell - 1 >= 0 &&
		   m_Board[row][col - amountLeftOfCell - 1].item<int>() == player) {
		amountLeftOfCell++;
	}
	while (col + amountRightOfCell + 1 < m_Cols &&
		   m_Board[row][col + amountRightOfCell + 1].item<int>() == player) {
		amountRightOfCell++;
	}
	...
	while (row - amountAboveCell - 1 >= 0 &&
		   m_Board[row - amountAboveCell - 1][col].item<int>() == player) {
		amountAboveCell++;
	}
	while (row + amountBelowCell + 1 < m_Rows &&
		   m_Board[row + amountBelowCell + 1][col].item<int>() == player) {
		amountBelowCell++;
	}
	...
	while (
		row - amountDiagonalLeftUp - 1 >= 0 &&
		col - amountDiagonalLeftUp - 1 >= 0 &&
		m_Board[row - amountDiagonalLeftUp - 1][col - amountDiagonalLeftUp - 1]
				.item<int>() == player) {
		amountDiagonalLeftUp++;
	}
	while (row + amountDiagonalRightDown + 1 < m_Rows &&
		   col + amountDiagonalRightDown + 1 < m_Cols &&
		   m_Board[row + amountDiagonalRightDown + 1]
				  [col + amountDiagonalRightDown + 1]
					  .item<int>() == player) {
	...
	while (row + amountDiagonalLeftDown + 1 < m_Rows &&
		   col - amountDiagonalLeftDown - 1 >= 0 &&
		   m_Board[row + amountDiagonalLeftDown + 1]
				  [col - amountDiagonalLeftDown - 1]
					  .item<int>() == player) {
		amountDiagonalLeftDown++;
	}
	while (row - amountDiagonalRightUp - 1 >= 0 &&
		   col + amountDiagonalRightUp + 1 < m_Cols &&
		   m_Board[row - amountDiagonalRightUp - 1]
				  [col + amountDiagonalRightUp + 1]
					  .item<int>() == player) {
...
void Environment::printHistory() {
	...
	for (int i = 0; i < (int)m_BoardHistory.size(); i++) {
		...
		ss << cell.getRow() << " " << cell.getCol() << " "
		   << static_cast<int>(cell.getPlayer()) << std::endl;
	}
	LOG(INFO) << ss.str();
}