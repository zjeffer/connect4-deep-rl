...
NeuralNetwork::NeuralNetwork(Settings* selfPlaySettings) {
	...
	m_Net = Network(selfPlaySettings->getInputPlanes(), selfPlaySettings->getRows(), selfPlaySettings->getCols(),
					selfPlaySettings->getOutputSize(), 256, 2, 1);
...
Network NeuralNetwork::getNetwork() const { return m_Net; }
...
torch::Tensor NeuralNetwork::boardToInput(const std::shared_ptr<Environment>& env) {
	return NeuralNetwork::boardToInput(env->getBoard().detach().clone(), static_cast<int>(env->getCurrentPlayer()),
									   m_Settings->getInputPlanes())
		.to(m_Device);
}

std::pair<torch::Tensor, torch::Tensor> NeuralNetwork::predict(torch::Tensor& input) { return m_Net->forward(input); }
